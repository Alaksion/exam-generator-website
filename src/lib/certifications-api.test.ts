  ...
  it("GETs a certification by id", async () => {
    ...
    server.use(
      http.get("/v1/certifications/:id", ({ params }) => {
        capturedPath = new URL(`/v1/certifications/${params.id}`, "http://test")
          .pathname;
...
describe("updateCertification", () => {
  it("PUTs only the mutable fields to /v1/certifications/:id", async () => {
    ...
    server.use(
      http.put("/v1/admin/certifications/:id", async ({ params, request }) => {
        capturedPath = new URL(
          `/v1/admin/certifications/${params.id}`,
          "http://test",
        ).pathname;
    ...
    expect(capturedPath).toBe(
      "/v1/admin/certifications/c0000000-0000-0000-0000-000000000001",
    );
  ...
  it("throws ApiRequestError with status 404 when the certification is unknown", async () => {
    server.use(
      http.put("/v1/admin/certifications/:id", () =>
        HttpResponse.json(
          { error: "NotFound", message: "Certification not found." },
          { status: 404 },
        ),
    ...
    const err = await updateCertification("nope", update).catch(
      (e: unknown) => e,
    );
    expect(err).toBeInstanceOf(ApiRequestError);
    expect((err as ApiRequestError).status).toBe(404);
  ...
  it("throws ApiRequestError with status 400 on an invalid request", async () => {
    ...
    const err = await updateCertification(cert.id, update).catch(
      (e: unknown) => e,
    );
    expect(err).toBeInstanceOf(ApiRequestError);
