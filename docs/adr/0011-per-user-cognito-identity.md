...
Consequences: the login (SRP email/password) UI is pulled into this slice; signup, forgot-password, and Google/Apple social land later against the same Cognito pool. The ID/Access tokens live in memory with Amplify owning token refresh from its own storage; a `localStorage` marker records that a session existed so a reload (or a new tab) can restore it and skip the login prompt. On `401` the app refreshes once and retries, then forces re-login. Admin capabilities are gated in the UI (nav + route guard); the backend enforces roles with `403` regardless of the UI.
